...
class ExpirePoints extends Base
{
    ...
    function creditInsert($earn_tran, $args)
    {
        ...
        if ($enable_expire_point) {
            ...
            if ($expire_after > 0) {
                ...
                $credit_fields['expire_date'] = strtotime(date("Y-m-d", strtotime("+" . $expire_after . " " . $expire_period)));
            ...
            if ($enable_expire_email) {
                ...
                if ($expire_email_after > 0 && $expire_after > 0) {
                    ...
                    $credit_fields['expire_email_date'] = strtotime(date("Y-m-d", strtotime("+" . $expire_email_days . " " . $expire_email_period)));
                }
            }
        }
    ...
    function getUpcomingExpirePointForCustomer($user_email, $expire_points = 30, $expire_range_type = 'days')
    {
        ...
        $upcoming_date = strtotime(date('Y-m-d', strtotime('+' . $expire_points . ' ' . $expire_range_type)));
