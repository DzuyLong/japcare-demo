    ...
    function updateExpiryDate()
    {
        ...
        if ($row_id > 0 && !empty($action_type) && in_array($action_type, array('point', 'email'))) {
            ...
            if ($action_type === 'point') {
                $update_date = array('expire_date' => self::$woocommerce->beforeSaveDate($expiry_point_date), 'modified_at' => $current_date);
                $message['data'] = array(
                    'message' => __('Point expiry date updated successfully', 'wp-loyalty-rules')
                );
            } elseif ($action_type === 'email') {
                $update_date = array('expire_email_date' => self::$woocommerce->beforeSaveDate($expiry_email_date), 'modified_at' => $current_date);
                $message['data'] = array(
