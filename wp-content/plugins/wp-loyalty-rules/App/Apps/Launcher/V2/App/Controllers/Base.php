...
class Base
{
    ...
    public function getUserDetails()
    {
        ...
        return self::$current_user_details = $customer_page->getPageUserDetails($user_email);
    }
    ...
    function getUserRewards()
    {
        ...
        $user_email = self::$woocommerce->get_login_user_email();
        $customer_page = new \Wlr\App\Controllers\Site\CustomerPage();
        $user_rewards = $customer_page->getPageUserRewards($user_email);
    ...
    function getUserCouponText($user_reward)
    {
        ...
        switch ($discount_type) {
            case 'fixed_cart':
            case 'points_conversion':
                $text = self::$woocommerce->getCustomPrice($discount_value);
                break;
            case 'percent':
    ...
    function getRewardOpportunities()
    {
        ...
        if (empty($rewards) || !is_array($rewards)) return array('reward_opportunity' => array(), 'message' => __('No rewards found!', 'wp-loyalty-rules'));
        ...
        $message = "";
        if (count($rewards) == 0) {
            $message = __('No rewards found!', 'wp-loyalty-rules');
        }
