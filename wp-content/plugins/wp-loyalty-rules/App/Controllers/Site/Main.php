...
class Main extends Base
{
    ...
    function addFrontEndScripts()
    {
        ...
        $localize = array(
            ...
            'revoke_coupon_message' => __("Are you sure you want to return the reward ?", 'wp-loyalty-rules'),
        ...
        if ($table === 'user_reward') {
            ...
            try {
                ...
                if ($user_reward_id > 0) {
                    ...
                    if (isset($user_reward->reward_currency) && $user_reward->reward_currency != get_woocommerce_currency()) {
    ...
    function changeFreeProductPrice()
    {
        ...
        if (!empty($cart_items)) {
            foreach ($cart_items as $key => $item) {
                if (isset($item['loyalty_free_product']) && $item['loyalty_free_product'] == 'yes') {
                    if (!empty($item["data"])) {
                        self::$woocommerce->setCartProductPrice($item["data"], 0);
                    }
                }
            }
        }
    ...
    function emailUpdatePointTransfer($status, $old_user_data, $new_user_data)
    {
        ...
        $user_email = self::$woocommerce->get_login_user_email();
        if (isset($new_user_data['user_email']) && isset($old_user_data['user_email']) && !empty($new_user_data['user_email']) && $old_user_data['user_email'] !== $new_user_data['user_email'] && $user_email == $old_user_data['user_email']) {
            $earn_campaign_helper = new EarnCampaign();
