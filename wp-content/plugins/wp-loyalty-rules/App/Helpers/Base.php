...
class Base
{
    ...
    {
        $earning = array();
        if (!$this->is_valid_action($action_type) || !$this->isEligibleForEarn($action_type, $extra)) {
            return $earning;
        }
    ...
    function getUserTotalTransactionAmount($user_email)
    {
        ...
        if (!empty($total_trans)) {
            $tax_including = wc_tax_enabled() && 'yes' === get_option('woocommerce_prices_include_tax');
            foreach ($total_trans as $transaction) {
                if (isset($total_amount_list[$transaction->reward_currency]) && !empty($total_amount_list[$transaction->reward_currency])) {
                    if (isset($total_amount_list[$transaction->reward_currency]['order_total'])) {
                        $total_amount_list[$transaction->reward_currency]['order_total'] += $transaction->order_total;
                    } else {
                        $total_amount_list[$transaction->reward_currency]['order_total'] = $transaction->order_total;
                    }
                    if (isset($total_amount_list[$transaction->reward_currency]['reward_amount'])) {
                        $total_amount_list[$transaction->reward_currency]['reward_amount'] += $transaction->reward_amount;
                    } else {
                        $total_amount_list[$transaction->reward_currency]['reward_amount'] = $transaction->reward_amount;
                    }
                    if ($tax_including && isset($transaction->reward_amount_tax) && $transaction->reward_amount_tax > 0) {
                        $total_amount_list[$transaction->reward_currency]['reward_amount'] += $transaction->reward_amount_tax;
                    }
                } else {
                    $total_amount_list[$transaction->reward_currency] = array(
                        'order_total' => $transaction->order_total,
                        'reward_amount' => $tax_including ? ($transaction->reward_amount + $transaction->reward_amount_tax) : $transaction->reward_amount,
                        'currency_symbol' => $transaction->reward_currency,
                    );
                }
            }
        }
        foreach ($total_amount_list as &$total_amount) {
            $total_amount['display_format'] = wc_price($total_amount['reward_amount'], array('currency' => $total_amount['currency_symbol']));
        }
        return $total_amount_list;
    }
...
' . $svg_file . '<p style="margin: 0 0 0;">' . $message . '</p></div>';
    ...
    function getNextLevel($to_point = 0)
    {
        ...
        return (new Levels())->getQueryData($level_where_data);
    }
