...
(function (wlr) {
    ...
    wlr_jquery(document).on('revertEnable', function (e, id) {
        wlr_jquery(".wlr-myaccount-page .wlr-customer-reward .wlr-revert-tool i").toggleClass("wlrf-arrow-down wlrf-arrow_right");
    ...
    let click_status = [];
    wlr_jquery(document).on('wlr_apply_reward_action', function (e, id, type, button_id = "") {
    ...
    wlr_jquery(document).on('wlr_apply_point_conversion_reward', function (e, id, type, point, input_id = "", button_id = "") {
        if (button_id !== "") {
            wlr.disableButton(button_id);
        }
