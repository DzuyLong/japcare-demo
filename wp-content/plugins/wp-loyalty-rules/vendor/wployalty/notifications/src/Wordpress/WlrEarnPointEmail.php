...
class WlrEarnPointEmail extends \WC_Email
{
    ...
    function __construct()
    {
        ...
        $this->default_subject = __('You have earned points.', 'wp-loyalty-rules');
    ...
    function sendPointEmail($email, $point, $action_type, $data)
    {
        ...
        if (!empty($email) && $reward_helper->is_valid_action($action_type) && $this->isEligibleForSentEmail($action_type,$data)) {
            ...
            $short_codes = apply_filters('wlr_earn_point_email_short_codes', $short_codes,$email, $point, $action_type, $data);
            ...
            $this->add_placeholder('{wlr_earn_point_mail_content}', $content_html);
            ...
            if ($this->is_enabled()) {
                ...
                $log_data['note'] = sprintf(__('Sending earned %s email failed(%s)', 'wp-loyalty-rules'), $point_label, $campaign_name);
                $log_data['customer_note'] = sprintf(__('Sending earned %s email failed to %s for %s campaign', 'wp-loyalty-rules'), $point_label, $email, $campaign_name);
                if ($this->send($this->get_recipient(), $subject, $this->get_content(), $this->get_headers(), $attachments)) {
                    $log_data['note'] = sprintf(__('Earned %s email sent to customer successfully(%s)', 'wp-loyalty-rules'), $point_label, $campaign_name);
                    $log_data['customer_note'] = sprintf(__('Earned %s email sent to %s for %s campaign', 'wp-loyalty-rules'), $point_label, $email, $campaign_name);
                }
